...
class HeyboxScraper:
    ...
    @classmethod
    def _log_fast_outcome(cls, url: str, stage: str, *, result: dict[str, Any] | None = None, captcha: bool = False) -> None:
...
class _NodriverBrowserWorker:
    ...
    async def _launch_browser(*, headless: bool):
        ...
        return await nodriver.start(
            user_data_dir=HeyboxScraper._profile_dir(),
            headless=headless,
    ...
    async def _refresh_fast_state(self, browser, tab) -> None:
        ...
        try:
            ...
            cookies = [
                ...
                if HeyboxScraper._is_fast_cookie_domain(
                    HeyboxScraper._state_value(getattr(cookie, "domain", None), max_length=200)
                )
            ]
        except Exception:
            cookies = []
        ...
        HeyboxScraper._save_browser_session_snapshot(
            cookies=cookies,
            browser_snapshot=browser_snapshot,
        )
    ...
    async def _extract_dom_result(self, tab, url: str) -> dict[str, Any] | None:
        ...
        videos = HeyboxScraper._collect_video_urls(payload.get("videos"))
        meta = HeyboxScraper._clean_meta(
            {
                "title": HeyboxScraper._state_value(payload.get("title"), max_length=120) or "",
                "description": (
                    HeyboxScraper._state_value(payload.get("description"), max_length=260) or ""
                ),
            }
        )
        result = HeyboxScraper._make_result(videos, meta, source="browser")
        if result:
            LOGGER.info(
                "heybox parse nodriver success link_id=%s stage=dom_fallback",
                HeyboxScraper._extract_link_id(url) or "-",
            )
        return result
    ...
    async def _scrape_with_browser(
        ...
        *,
        ...
        link_id = HeyboxScraper._extract_link_id(url) or "-"
        await tab.get(url)
        await tab.wait(PLAYWRIGHT_TIMEOUT / 1000)
        await self._refresh_fast_state(browser, tab)

        result = HeyboxScraper._scrape_fast(url)
        if result:
        ...
        if captcha_required:
            ...
            raise CaptchaRequiredError("灏忛粦鐩掕繑鍥為獙璇佺爜锛屾棤娉曡嚜鍔ㄨВ鏋愯甯栧瓙")
    ...
    async def _run_async(self) -> None:
        ...
        try:
            browser = await self._launch_browser(headless=current_headless)
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    finish(error=error)
                    if HeyboxScraper._is_browser_closed_error(error):
                        break
                finally:
...
class _BrowserWorker:
    ...
    @staticmethod
    def _launch_context_once(playwright):
            ...
            try:
                ...
                while True:
                    ...
                    try:
                        ...
                        finish(error=e)
                        if HeyboxScraper._is_browser_closed_error(e):
                            break
                    except Exception as e:
                        finish(error=e)
            finally:
